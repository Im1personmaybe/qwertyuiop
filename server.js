...
// Parse bodies for non-proxy routes
  ...
  return new Promise((resolve, reject) => {
    ...
    const proxyReq = client.request(options, (proxyRes) => {
      ...
      Object.entries(proxyRes.headers).forEach(([key, value]) => {
        ...
        if (k === 'location') { res.setHeader(key, rewriteLocation(value, targetUrl, req)); return; }
...
app.all('/proxy', async (req, res) => {
  ...
  try { await proxyPass(target, req, res, { rewriteUrls: req.query.rewrite === 'true' }); } catch (e) { console.error(e); }
});
